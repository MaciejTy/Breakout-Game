    ...
    private AnimationTimer animationTimer = new AnimationTimer() {
        ...
        public void handle(long now) {
            ...
            for (int i = 0; i < bricks.size(); i++) {
                ...
                String value = String.valueOf(brick.checkCollision(ball.getTopLeft(), ball.getBottomLeft(), ball.getTopRight(), ball.getBottomRight()));

                if (value.equals("VerticalCrush")) {
                    ball.bounceVertically();
                    bricks.remove(brick);
                }

                if (value.equals("HorizontalCrush")) {
